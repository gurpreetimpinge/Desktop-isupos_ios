...
+ (NSString*) createEnvelope: (NSString*) method forNamespace: (NSString*) ns forParameters: (NSString*) params withHeaders: (NSDictionary*) headers
{
    ...
    if (appDelegate.bool_RegisterSale)
    {
        [s appendFormat: @"<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:tem=\"http://tempuri.org/\" xmlns:aff=\"http://schemas.datacontract.org/2004/07/Affarsit.Lagerkoll.CentralServiceLibrary\">"];
    }
    else
    {
       [s appendFormat: @"<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:tem=\"http://tempuri.org/\">"];
    }
	...
	[s appendFormat: @"<tem:%@>%@</tem:%@>", method,[params stringByReplacingOccurrencesOfString:@"&" withString:@"&amp;"], method];
	[s appendString: @"</soapenv:Body>"];
...
+(NSString*)serializeHeader:(NSDictionary*)headers forKey:(NSString*)key {
	...
	return [NSString stringWithFormat:@"<%@>%@</%@>", key, temp, key];
}
...
// Creates the XML request for the SOAP envelope with optional SOAP headers.
+ (NSString*) createCustomeEnvelope: (NSString*) method forNamespace1: (NSString*) ns1 forNamespace2: (NSString*) ns2  withParameters: (NSArray*) params withHeaders: (NSDictionary*) headers
{
    NSMutableString* s = [[[NSMutableString alloc] initWithString: @""] autorelease];
    for(SoapParameter* p in params) {
        [s appendString: p.xml];
    }
    NSString* envelope = [Soap createCustomeCostEnvelope: method forNamespace1: ns1 forNamespace2: (NSString*) ns2  forParameters: s withHeaders: headers];
    return envelope;
}


// Creates the XML request for the SOAP envelope with optional SOAP headers.
+ (NSString*) createCustomeCostEnvelope: (NSString*) method forNamespace1: (NSString*) ns1 forNamespace2: (NSString*) ns2 forParameters: (NSString*) params withHeaders: (NSDictionary*) headers
{
    NSMutableString* s = [[NSMutableString string] autorelease];
...
/*    if(headers != nil && headers.count > 0) {
    ...
    */
    ...
    [s appendString: @"<soapenv:Body>"];
    [s appendFormat: @"<mob:%@>%@</mob:%@>", method,[params stringByReplacingOccurrencesOfString:@"&" withString:@"&amp;"], method];
    [s appendString: @"</soapenv:Body>"];
    [s appendString: @"</soapenv:Envelope>"];
    return s;
}
...
+ (NSString*) serialize: (id) object withName: (NSString*) nodeName {
    ...
    AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    if (appDelegate.bool_RegisterSale && !([nodeName isEqualToString:@"username"] || [nodeName isEqualToString:@"password"] || [nodeName isEqualToString:@"receipt"]))
    {
        return [NSString stringWithFormat:@"<aff:%@>%@</aff:%@>", nodeName, [Soap serialize: object], nodeName];
    }
    else
    {
        return [NSString stringWithFormat:@"<tem:%@>%@</tem:%@>", nodeName, [Soap serialize: object], nodeName];
    }
