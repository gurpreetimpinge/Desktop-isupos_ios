...
+ (SMBluetoothManager *)loadBluetoothSetting:(NSString *)portName portSettings:(NSString *)portSettings {
    ...
    if ([portName rangeOfString:@"BT:" options:NSCaseInsensitiveSearch range:NSMakeRange(0, 3)].location == NSNotFound) {
        alert.message = [Language get:@"This function is available via the bluetooth interface only." alter:@"!This function is available via the bluetooth interface only."];
        [alert show];
        return nil;
    }
    ...
    if (manager == nil) {
        alert.message =[Language get:@"initWithPortName:deviceType: is failure." alter:@"!initWithPortName:deviceType: is failure."] ;
        [alert show];
        return nil;
    }
    
    if ([manager open] == NO) {
        alert.message = [Language get:@"open is failure." alter:@"!open is failure."];
        [alert show];
        return nil;
    }
    
    if ([manager loadSetting] == NO) {
        alert.message =[Language get:@"loadSetting is failure." alter:@"!loadSetting is failure."] ;
        [alert show];
        [manager close];
        return nil;
    }
...
+ (void)disconnectPort:(NSString *)portName portSettings:(NSString *)portSettings timeout:(u_int32_t)timeout {
    ...
    if (result == NO) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[Language get:@"Fail to Disconnect" alter:@"!Fail to Disconnect"] message:@"" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [alert show];
        [alert release];
        return;
    }
