...
@synthesize animationType, opacity, labelFont, detailsLabelFont, delegate, indicator, width, height, xOffset, yOffset, margin, graceTime, minShowTime, graceTimer, minShowTimer, taskInProgress, removeFromSuperViewOnHide, customView, showStarted;
...
- (id) initWithView:(UIView *) view
{
	...
	if (!view)
    {
		[NSException raise:@"MBProgressHUDViewIsNillException" format:@"The view used in the MBProgressHUD initializer is nil."];
	}
...
//        CGSize dims = [self.labelText sizeWithFont:self.labelFont];
        ...
        CGRect lFrame = CGRectMake(floor((frame.size.width - lWidth) / 2) + xOffset, floor(indFrame.origin.y + indFrame.size.height + PADDING), lWidth, lHeight);
...
//            dims = [self.detailsLabelText sizeWithFont:self.detailsLabelFont];
			CGSize dims = [self.labelText sizeWithAttributes: @{NSFontAttributeName: self.detailsLabelFont}];
...
- (void) hide:(BOOL) animated
{
	...
	if (self.minShowTime > 0.0 && showStarted)
    {
        ...
        {
			self.minShowTimer = [NSTimer scheduledTimerWithTimeInterval:(self.minShowTime - interv) target:self selector:@selector(handleMinShowTimer:) userInfo:nil
                repeats:NO];
...
- (void) drawRect:(CGRect) rect
{
    ...
    CGRect boxRect = CGRectMake(((allRect.size.width - self.width) / 2) + self.xOffset, ((allRect.size.height - self.height) / 2) + self.yOffset, self.width, self.height);
